#!/usr/bin/env bash
# Common helpers for mysql-lab test suite
...
SERVER_PORT="$(_get_port mysql-lab)"
if [[ -z "$SERVER_PORT" ]]; then echo "ERROR: Cannot find VM port. Is mysql-lab running?"; exit 1; fi
...
mysql_query() { ssh_vm "mysql -u labuser -plabpass -N -e \"$1\""; }
mysql_query_db() { ssh_vm "mysql -u labuser -plabpass -N $1 -e \"$2\""; }

cleanup_mysql() {
    log_info "Cleaning up mysql test artifacts..."
    ssh_vm "mysql -u labuser -plabpass -e \"DROP DATABASE IF EXISTS testlab; DROP USER IF EXISTS 'reader'@'localhost';\" 2>/dev/null; rm -f /tmp/testdb_backup.sql" 2>/dev/null || true
}
